...
ssize_t write_uid(struct file* filp, const char* buffer, size_t count,
    ...
    char* holder_uid = kmalloc(16 * sizeof(char), GFP_KERNEL);
...
ssize_t read_toggle(struct file* filp, char* buffer, size_t count,
                    loff_t* offp) {
    char* holder_toggle = kmalloc(16 * sizeof(char), GFP_KERNEL);
    if (temp_toggle == 0) {
...
ssize_t write_toggle(struct file* filp, const char* buffer, size_t count,
    ...
    char* holder_toggle = kmalloc(2 * sizeof(char), GFP_KERNEL);
...
const struct file_operations proc_sysmon_log = {read : read_log};
...
int proc_init(void) {
    init_proc_entries();
    init_log();
    if (toggle == 1) monitor_init(&monitor_handler);
    return 0;
}
